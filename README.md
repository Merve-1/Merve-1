# 👩‍💻 Marwa Alazab  

Welcome to my GitHub profile! I am a **Software Engineer** focused on building full-stack applications with a strong emphasis on secure design and cybersecurity principles. I am currently pursuing a master’s degree in **Cybersecurity of AI**, where I strengthen my practical skills in penetration testing, vulnerability assessment, and security research.

I am passionate about identifying security weaknesses, solving Capture The Flag (CTF) challenges, and continuously improving my technical knowledge through hands-on labs and real-world security scenarios.

---

## 🛡️ Areas of Interest

- Web development
- Penetration Testing  
- Web Application Security  
- Vulnerability Assessment  
- Secure Software Development  
- Threat Detection & Incident Response  

---

## 🚀 Technical Skills

### Programming & Development
- Angular, React  
- Spring Boot, .NET, Laravel  
- Java, C#, Php, C, Python  

### Cybersecurity
- Web Application Testing  
- Security Fundamentals  
- CTF Challenges  
- Linux & Networking Basics  
- Secure Coding Practices  

---

## 📂 Featured Projects

- **Frontend Applications** — Angular & React projects  
- **Backend Development** — Spring Boot, .NET, and Laravel applications  
- **Cybersecurity Labs & CTFs** — Practical security exercises and challenge writeups  
- **Java Projects** — SIC/XE Assembler, Discrete Event Simulation  
- **C Projects** — Student Database System  
- **Blockchain Applications** — Decentralized Voting Application  
- **IoT Security Projects** — Secure Home System & Pharmaceutical Cargo Monitoring  
- **AI & Machine Learning Projects**  
  - Tic-Tac-Toe Heuristic Calculator  
  - M&M Counter using Computer Vision  
  - Face Image Classification (Extended Yale B Dataset)  
  - Tweet Sentiment Analysis using RNN & LSTM  

---

## 🏆 Cybersecurity Profiles

[![TryHackMe](https://img.shields.io/badge/TryHackMe-0xMerve-red?style=for-the-badge&logo=tryhackme&logoColor=white)](https://tryhackme.com/p/0xMerve)
[![Hack The Box](https://img.shields.io/badge/Hack%20The%20Box-Profile-green?style=for-the-badge&logo=hackthebox&logoColor=white)](https://profile.hackthebox.com/profile/019e5b48-8972-7270-b84c-cdfaa6bdcb1b)

---

## 📫 Contact

<p align="left">
  <a href="mailto:marwa.alazaab@gmail.com">
    <img src="https://img.shields.io/badge/Email-marwa.alazaab%40gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://www.linkedin.com/in/marwa-alazab/">
    <img src="https://img.shields.io/badge/LinkedIn-Marwa%20Alazab-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
</p>
